<template>
  <v-card
    ...
    :id="post.id"
    @click="openPost(post)"
    :style="post.id === selectedIndex ? 'border:2px solid red' : ''"
  >
    <v-card-title class="pb-0 pt-1">
      <v-list-item-avatar color="grey darken-3">
        <v-img class="elevation-6" :src="post.user.avatar"></v-img>
      </v-list-item-avatar>
      <span class="title font-weight-bold" style="font-size:16px !important;"
    ...
    <v-card-actions class="mt-n5">
      <v-list-item class="">
        <v-row align="center" justify="end" class="post_info ml-n8">
          ...
          <span class="subheading mr-3 mt-1" style="color:#707C97 !important">{{

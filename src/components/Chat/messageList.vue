<template>
  <v-list id="messageList" height="" :style="chatStyle" rounded class="scroll">
    <v-list-item-group>
      <ul v-for="(block, block_index) in blocks" :key="block_index">
        <div v-for="(message, message_index) in block" :key="message_index">
          ...
          <separator
            ...
            v-if="
              message_index > 0 &&
                diff_hours(message.date, block[message_index - 1].date) > 3
            "
          >
            {{
              block[message_index - 1].date | moment("from", "now")
            }}</separator
          >
          <!-- sent message !-->
          <chatBubble
...
export default {
  ...
  methods: {
    ...
    isFirst(prev_message, message) {
      ...
      if (
        prev_message.sender.id == this.loggedUser &&
        message.sender.id != this.loggedUser
      ) {
        return true;
      } else if (
        prev_message.sender != this.loggedUser &&
        message.sender == this.loggedUser
      ) {
        return true;
      } else {
        return false;
    ...
    isLast(message, next_message) {
      ...
      if (
        message.sender.id == this.loggedUser &&
        next_message.sender.id != this.loggedUser
      ) {
        return true;
      } else if (
        message.sender != this.loggedUser &&
        next_message.sender == this.loggedUser
      ) {
        return true;
      } else {
        return false;

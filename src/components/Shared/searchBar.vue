<template>
  <v-card elevation="5" tile class="searchBar">
    <v-layout>
      ...
      <v-divider class="divider my-0" inset vertical v-if="posts"></v-divider>

      <v-flex v-if="posts" class="mt-2 sortBy">
        <v-menu close-on-content-click :offset-y="true">
          <template v-slot:activator="{ on }">
            <v-btn color="#777" style="text-transform:none;" text v-on="on">
              Sort By
          ...
          <v-list offset-y="10">
            ...
            >
              <v-list-item-title>
                <span class="clickable" @click="sortItems(item.sortBy)"
                  ><v-icon>{{ item.icon }}</v-icon> {{ item.title }}</span
                >
              </v-list-item-title>
            </v-list-item>
          </v-list>

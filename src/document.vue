    ...
    <div id="app">
      <v-container>
        <v-layout row wrap justify-space-between>
          <v-flex
            d-flex
            v-if="!expandRight || isClosed"
            :class="breakPointLeft"
          >
            <div class="left_layout">
              <div class="header_titles">
                ...
                <h3 class="header_subtitle">
                  <span v-if="!isPrivate"
                    >Public posts
                    <v-icon class="header_icon  ml-1" size="18"
                      >mdi-earth</v-icon
                    >
                  </span>
                  <span v-else
                ...
                <v-layout>
                  <v-flex d-flex md8 lg12>
                    <v-icon class="mr-4 mb-1" size="36" style="color:#979797"
                      >mdi-earth</v-icon
                    >
                    <v-switch v-model="isPrivate" inset></v-switch>
          ...
          <v-flex d-flex :class="breakPointRight">
            <v-layout row wrap class="right_layout pl-0">
              <postPage
                v-show="showPost && !isClosed"
                @layoutPropertiesChanged="updateSize($event)"
                :class="expandRight ? 'mt-2' : ''"
                :document="opened_document"
                :loggedUser="loggedUser"
              />
              <draggable style="width:100%">
                <chatbox
                  v-show="showChat && !isClosed"
                  @layoutPropertiesChanged="updateSize($event)"
                  :class="expandRight ? 'mt-2' : ''"
                  :document="opened_document"
                  :loggedUser="loggedUser"

<template>
  <span>
    ...
    <v-list id="discussionList" class="list_layout">
      ...
      <draggable
        v-model="filtered_discussions"
        group="discussion"
        :move="checkMove"
      >
        <v-list-item-group
          v-for="discussion in filtered_discussions"
          :key="discussion.id"
        >
          <v-card
            class="discussion_card pa-2"
            :id="discussion.id"
            @click="openChat(discussion)"
            :class="
              discussion.id === selectedIndex ? 'selected_discussion' : ''
            "
          >
            <v-card-title class="pb-0 pt-1">
              ...
              <v-list-item-avatar v-else color="grey darken-3">
                <v-img
                  class="elevation-6"
                  :src="
                    discussion.receivers[discussion.receivers.length - 1].avatar
                  "
                ></v-img>
              </v-list-item-avatar>
              <span
              ...
              <span
                ...
                class="title font-weight-bold"
                style="font-size:16px !important;"
                >{{ discussion.receivers[0].name }}
                {{ discussion.receivers[0].lastName }}</span
              >
              <v-spacer></v-spacer>
              <span class="discussion_date font-weight-medium">{{
                discussion.date | moment("from", "now")
              }}</span>
            </v-card-title>

            <v-layout>
              <v-card-text align="left">
                <span v-if="not_empty(discussion)">
                  <span style="color:#60A9F6">
              ...
              >
                <span style="margin-top:15%" align="end"> 1 </span>
              </v-avatar>
            </v-layout>
          </v-card>
        </v-list-item-group>
  ...
  methods: {
    getUserDiscussions() {
      ...
      var discussions = this.document.discussions.filter(function(discussion) {
        ...
        discussion.receivers.forEach(receiver => {
          if (receiver.id === that.loggedUser.id) {
            isReceiver = true;
          }
        });
        return isReceiver;
    ...
    last_sender(discussion) {
      if (discussion.messages[discussion.messages.length - 1].sender.name)
        return (
          discussion.messages[discussion.messages.length - 1].sender.name + ":"
        );
    },
    ...
    EventBus.$on("createChat", function(discussion) {
      ...
      if (!that.document.discussions.includes(discussion)) {
        ...
        that.discussionService
          .createDiscussion(that.document)
          .then(() => {

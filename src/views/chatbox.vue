  ...
  <v-container ref="chatbox" style="" class="no_padding no_margin">
    <v-col :cols="windowProperties.width" class="fill-height" style="">
      <baseCard class=" mb-n2">
        ...
        <!-- Message List !-->
        <messageList
          v-if="discussion.messages.length > 0"
          ref="infoBox"
        ...
        <chatInput
          ...
          @sendMessage="submitMessage($event)"
...
export default {
  ...
  methods: {
    ...
    submitMessage(newMessage) {
      ...
      var discussionIndex = this.document.discussions.findIndex(
        discussion => discussion.id == this.discussion.id
      );
      newMessage.id =
        this.document.discussions[discussionIndex].messages.length + 1;
      ...
      if (
        message.sender != last_message.sender ||
        this.diff_hours(message.date, last_message.date) < 3
      ) {
    ...
    setBlocks() {
      ...
      for (var i = 1; i < this.discussion.messages.length; i++) {
        if (
          this.discussion.messages[i - 1].sender.id ==
          this.discussion.messages[i].sender.id
        ) {
          let last_block = this.message_blocks[this.message_blocks.length - 1];
    ...
    updateSize(newProperties) {
      ...
      this.$emit("layoutPropertiesChanged", {
        isExpanded: this.windowProperties.isExpanded,
        isClosed: this.windowProperties.isClosed,
        component: "chatbox"
      });
      EventBus.$emit("layoutPropertiesChanged", {
        isExpanded: this.windowProperties.isExpanded,
        isClosed: this.windowProperties.isClosed,
        component: "chatbox"
      });
    ...
    refreshMessages() {
      this.messageService
        .getMessages(this.discussion.id, this.discussion)
        .then(messageList => {
    ...
    removeMember(member) {
      const that = this;
      var discussionIndex = this.document.discussions.findIndex(
        discussion => discussion.id == that.discussion.id
      );
      var receiverIndex = this.document.discussions[
        discussionIndex
      ].receivers.findIndex(receiver => receiver.id == member.id);
      this.document.discussions[discussionIndex].receivers.splice(
        receiverIndex,
        1
      );
  ...
  computed: {
    last_message_date() {
      if (this.discussion.messages.length > 0)
        return this.discussion.messages[this.discussion.messages.length - 1]
          .date;
      else return null;
    }
  },
  mounted() {
    ...
    EventBus.$on("updateMessage", function(updatedMessage) {
      var discussionIndex = that.document.discussions.findIndex(
        discussion => discussion.id == that.discussion.id
      );
      var messageIndex = that.document.discussions[
        discussionIndex
      ].messages.findIndex(message => message.id == updatedMessage.id);
      that.document.discussions[discussionIndex].messages[
        messageIndex
      ] = updatedMessage;
    ...
    EventBus.$on("deleteMessage", function(messageId) {
      ...
      that.document.discussions[discussionIndex].messages.splice(
        messageIndex,
        1
      );
    ...
    EventBus.$on("addMembers", function(receivers, title) {
      ...
      that.discussionService
        .updateDiscussion(that.document)
        .then(() => {

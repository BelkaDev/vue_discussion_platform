<template>
  <v-container>
    <baseCard class=" mb-n2">
      <v-layout
        class="post_detail fill-height"
        :style="post.comments.length == 0 ? 'height:600px' : ''"
      >
        <v-flex>
          <div>
            <postPageToolbar
            ...
            <div id="comment_input">
              <commentInput class="pt-8" @addComment="submitComment($event)" />
              <alertDialog
...
export default {
  ...
  methods: {
    ...
    submitComment(newComment) {
      ...
      var postIndex = this.document.posts.findIndex(
    ...
    refreshComments(postIndex) {
      const that = this;
      this.commentService
        .getComments(this.document.id, this.post.id)
        .then(comments => {
  ...
  computed: {
    checkLike() {
      if (
        this.post.likes.findIndex(like => like.user.id == this.loggedUser.id) ==
        -1
      )
        return false;
      else return true;
  ...
  created() {
    ...
    EventBus.$on("openPost", function(post) {
      ...
      that.post = post;
      var postIndex = that.document.posts.findIndex(
        post => post.id == that.post.id
      );
    ...
    EventBus.$on("setLike", function(like) {
      ...
      var isLiked = that.document.posts[postIndex].likes.findIndex(
        like => like.user.id == that.loggedUser.id
      );
      if (isLiked == -1) {
        that.document.posts[postIndex].likes.push(like);
        that.postService
          .setLike(that.document)
          .then(() => {
    ...
    EventBus.$on("unLike", function() {
      ...
      var likeIndex = that.document.posts[postIndex].likes.findIndex(
        like => like.user.id == that.loggedUser.id
      );
